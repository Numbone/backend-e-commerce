...
import { ConfirmationDto } from './dto/cofirmation.dto';
...
export class EmailConfirmationService {
    ...
    private async generateVerificationToken(email: string) {
        ...
        const existingToken = await this.prismaService.token.findFirst({ where: { email, type: TokenType.VERIFICATION } });
        if (existingToken) {
            await this.prismaService.token.delete({ where: { id: existingToken.id, type: TokenType.VERIFICATION } });
        }

        const verificationToken = await this.prismaService.token.create({ data: { email, token, expiredIn: expiresIn, type: TokenType.VERIFICATION } });
